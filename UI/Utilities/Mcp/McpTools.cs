...
internal static class McpTools
{
	public static readonly IReadOnlyList<McpToolDesc> Descriptions = new List<McpToolDesc> {
		...
		new("render_tile_sheet",
			"Render a region of VRAM as a tile sheet PNG using the chosen palette + "
				+ "BPP. Like Mesen's Tile Viewer but as one MCP call. Use to spot "
		...
		new("render_oam",
			"Render OAM as a PNG. mode='positioned' draws the 128 sprites at their "
				+ "current screen X/Y on a transparent canvas (the same view as Mesen's "
		...
		new("record_movie",
			"Start recording a Mesen movie (.mmo) to disk. Use 'from'='CurrentState' "
		...
		new("play_movie",
			"Start playing back a Mesen movie (.mmo). The emulator advances input "
		...
		new("trace_log",
			"Return the last N CPU instructions executed, each with PC, opcode bytes, "
				...
				+ "Mesen's TraceLogger ring buffer (30000 entries deep). Far cheaper than "
		...
		new("watch_addresses",
			"Watch a list of memory addresses for changes over N frames. Bypasses the "
				...
				+ "Mesen's PpuFrameDone notification. Returns a timeline of every change "
	...
	private static JsonNode BuildAudioFingerprintSchema()
	{
		...
		return new JsonObject {
			...
			["properties"] = new JsonObject {
				["path"] = new JsonObject {
					["type"] = "string",
					["description"] = "Path to a 16-bit PCM WAV file (the format Mesen's WaveRecord emits).",
				},
			},
		};
	}
	...
	public static JsonNode RenderTilemap(JsonNode? args)
	{
		...
		//   5     → "Main" composite (Mesen tab; Layer=4)
		//   6     → "Sub" composite (Layer=5)
		// This matches the dropdown ordering in Mesen's TilemapViewer for SNES.
		...
		// Pull the same inputs Mesen's own TilemapViewerViewModel uses.
	...
	public static JsonNode TraceLog(JsonNode? args)
	{
		...
		// format Mesen's TraceLogger UI uses, so the output reads identically:
	...
	public static JsonNode RecordMovie(JsonNode? args)
	{
		...
		if(!Enum.TryParse<Mesen.Interop.RecordMovieFrom>(fromStr, ignoreCase: true, out var from)) {
			throw new McpException(-32602,
				"from must be 'CurrentState', 'StartWithoutSaveData', or 'StartWithSaveData'");
		}
		if(Mesen.Interop.RecordApi.MovieRecording() || Mesen.Interop.RecordApi.MoviePlaying()) {
			throw new McpException(-32603, "another movie is already recording or playing — call stop_movie first");
		}
		var options = new Mesen.Interop.RecordMovieOptions(path, author, description, from);
		Mesen.Interop.RecordApi.MovieRecord(options);
		return new JsonObject {
			["path"] = path,
			["author"] = author,
			["from"] = from.ToString(),
	...
	public static JsonNode PlayMovie(JsonNode? args)
	{
		...
		Mesen.Interop.RecordApi.MoviePlay(path);
		return new JsonObject {
			["path"] = path,
			["playing"] = true,
		};
	...
	{
		bool wasRecording = Mesen.Interop.RecordApi.MovieRecording();
		bool wasPlaying = Mesen.Interop.RecordApi.MoviePlaying();
		if(wasRecording || wasPlaying) {
			Mesen.Interop.RecordApi.MovieStop();
		}
		return new JsonObject {
			["wasRecording"] = wasRecording,
			["wasPlaying"] = wasPlaying,
			["recording"] = Mesen.Interop.RecordApi.MovieRecording(),
			["playing"] = Mesen.Interop.RecordApi.MoviePlaying(),
		};
	}
	...
	// Minimal WAV reader: header + 16/8-bit PCM data chunk. Mesen's WaveRecord
	...
	public static JsonNode ReadMemory(JsonNode? args)
	{
		...
		// Mesen's DebugApi.GetMemoryValues(MemoryType.SnesMemory, ...) returns 0 for
