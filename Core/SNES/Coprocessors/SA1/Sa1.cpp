...
void Sa1::Sa1RegisterWrite(uint16_t addr, uint8_t value) {
	switch (addr) {
		...
		case 0x220C:
			_state.CpuNmiVector = (_state.CpuNmiVector & 0xFF00) | value;
			break; // SNV (SNES CPU NMI Vector - Low)
		case 0x220D:
			_state.CpuNmiVector = (_state.CpuNmiVector & 0xFF) | (value << 8);
			break; // SNV (SNES CPU NMI Vector - High)

		case 0x220E:
			_state.CpuIrqVector = (_state.CpuIrqVector & 0xFF00) | value;
			break; // SNV (SNES CPU IRQ Vector - Low)
		case 0x220F:
			_state.CpuIrqVector = (_state.CpuIrqVector & 0xFF) | (value << 8);
			break; // SIV (SNES CPU IRQ Vector - High)
		...
		case 0x2227:
			_state.Sa1BwWriteEnabled = (value & 0x80) != 0;
			break; // CBWE (SA-1 CPU BW-RAM Write Enable)
		case 0x222A:
			_state.Sa1IRamWriteProtect = value;
			break; // CIWP (SA-1 CPU I-RAM Write Protection)
		...
		case 0x2238:
			_state.DmaSize = (_state.DmaSize & 0xFF00) | value;
			break; // DTC (DMA terminal counter - Low)
		case 0x2239:
			_state.DmaSize = (_state.DmaSize & 0x00FF) | (value << 8);
			break; // DTC (DMA terminal counter - High)

		case 0x223F:
			_state.BwRam2BppMode = (value & 0x80) != 0;
			break; // BBF (Bitmap format)
		...
		case 0x2259:
			_state.VarLenAddress = (_state.VarLenAddress & 0xFFFF00) | value;
			break; // VDA (Variable length data address - Low)
		case 0x225A:
			_state.VarLenAddress = (_state.VarLenAddress & 0xFF00FF) | (value << 8);
			break; // VDA (Variable length data address - Mid)
		case 0x225B:
...
void Sa1::CpuRegisterWrite(uint16_t addr, uint8_t value) {
	switch (addr) {
		...
		case 0x2203:
			_state.Sa1ResetVector = (_state.Sa1ResetVector & 0xFF00) | value;
			break; // CRV (SA-1 Reset Vector - Low)
		case 0x2204:
			_state.Sa1ResetVector = (_state.Sa1ResetVector & 0xFF) | (value << 8);
			break; // CRV (SA-1 Reset Vector - High)

		case 0x2205:
			_state.Sa1NmiVector = (_state.Sa1NmiVector & 0xFF00) | value;
			break; // CRV (SA-1 NMI Vector - Low)
		case 0x2206:
			_state.Sa1NmiVector = (_state.Sa1NmiVector & 0xFF) | (value << 8);
			break; // CRV (SA-1 NMI Vector - High)

		case 0x2207:
			_state.Sa1IrqVector = (_state.Sa1IrqVector & 0xFF00) | value;
			break; // CRV (SA-1 IRQ Vector - Low)
		case 0x2208:
			_state.Sa1IrqVector = (_state.Sa1IrqVector & 0xFF) | (value << 8);
			break; // CRV (SA-1 IRQ Vector - High)

		case 0x2220:
			UpdateBank(0, value);
			break; // CXB (MMC Bank C)
		case 0x2221:
			UpdateBank(1, value);
			break; // DXB (MMC Bank D)
		case 0x2222:
			UpdateBank(2, value);
			break; // EXB (MMC Bank E)
		case 0x2223:
			UpdateBank(3, value);
			break; // FXB (MMC Bank F)
		...
		case 0x2226:
			_state.CpuBwWriteEnabled = (value & 0x80) != 0;
			break; // SBWE (SNES CPU BW-RAM Write Enable)

		case 0x2228:
			_state.BwWriteProtectedArea = (value & 0x0F);
			break; // BWPA (SNES CPU BW-RAM Write Protected Area)

		case 0x2229:
			_state.CpuIRamWriteProtect = value;
			break; // SIWP (SNES CPU I-RAM Write Protection)
	...
	switch (addr) {
		case 0x2231:
			...
			switch (_state.CharConvFormat) {
				case 0:
					_state.CharConvBpp = 8;
					break;
				case 1:
					_state.CharConvBpp = 4;
					break;
				case 2:
					_state.CharConvBpp = 2;
					break;
			}
		...
		case 0x2232:
			_state.DmaSrcAddr = (_state.DmaSrcAddr & 0xFFFF00) | value;
			break; // SDA (DMA source start address - Low) (Shared with SNES CPU)
		case 0x2233:
			_state.DmaSrcAddr = (_state.DmaSrcAddr & 0xFF00FF) | (value << 8);
			break; // SDA (DMA source start address - Mid) (Shared with SNES CPU)
		case 0x2234:
			_state.DmaSrcAddr = (_state.DmaSrcAddr & 0x00FFFF) | (value << 16);
			break; // SDA (DMA source start address - High) (Shared with SNES CPU)

		case 0x2235:
			_state.DmaDestAddr = (_state.DmaDestAddr & 0xFFFF00) | value;
			break; // DDA (DMA dest start address - Low) (Shared with SNES CPU)
		case 0x2236:
...
uint8_t Sa1::Sa1RegisterRead(uint16_t addr) {
	switch (addr) {
		...
		case 0x2302:
			break; // HCR (SA-1 H Counter read - Low)
		case 0x2303:
			break; // HCR (SA-1 H Counter read - High)

		case 0x2304:
			break; // VCR (SA-1 V Counter read - Low)
		case 0x2305:
			break; // VCR (SA-1 V Counter read - High)

		case 0x2306:
			ProcessMathOp();
			return _state.MathOpResult & 0xFF; // MR (Arithmetic result)
		case 0x2307:
			ProcessMathOp();
			return (_state.MathOpResult >> 8) & 0xFF; // MR (Arithmetic result)
		case 0x2308:
			ProcessMathOp();
...
uint8_t Sa1::CpuRegisterRead(uint16_t addr) {
	switch (addr) {
		...
		case 0x230E:
			break; // VC (Version code register)
	}
...
uint16_t Sa1::ReadVector(uint16_t vector) {
	switch (vector) {
		case Sa1Cpu::NmiVector:
			return _state.Sa1NmiVector;
		case Sa1Cpu::ResetVector:
			return _state.Sa1ResetVector;
		case Sa1Cpu::IrqVector:
			return _state.Sa1IrqVector;
	}
